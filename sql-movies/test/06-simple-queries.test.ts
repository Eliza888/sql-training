...
describe("Simple Queries", () => {
  ...
  it(
    ...
    async done => {
      const query = `
        SELECT
          SUM(budget_adjusted) AS total_budget,
          SUM(revenue_adjusted) AS total_revenue
        FROM
          movies;
      `;
      ...
      expect(result).toEqual({
        total_budget: 53668223285.93872,
        total_revenue: 148342748033.40323, 
      });
    ...
    async done => {
      const query = `
        ...
        FROM
          movies
        WHERE
          CAST(SUBSTR(release_date, 1, 4) AS INTEGER) > 2009
        ORDER BY
          budget DESC
        LIMIT
          3;
