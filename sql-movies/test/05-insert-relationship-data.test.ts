...
const insertMovieGenres = (
  ...
  const insertQueries = genres.map((genre) => {
    const matchingGenreRow = genreRows.find((row) => row.genre === genre);
    if (!matchingGenreRow) {
      throw new Error(`Genre not found: ${genre}`);
    }
    return `INSERT INTO ${MOVIE_GENRES} (movie_id, genre_id) VALUES (${movieId}, ${matchingGenreRow.id});`;
  });
...
const insertMovieActors = (
  ...
  const insertQueries = actors.map((actor) => {
    const matchingActorRow = actorRows.find((row) => row.full_name === actor);
    if (!matchingActorRow) {
      throw new Error(`Actor not found: ${actor}`);
    }
    return `INSERT INTO ${MOVIE_ACTORS} (movie_id, actor_id) VALUES (${movieId}, ${matchingActorRow.id});`;
  });
  return insertQueries.join("\n");
};
...
const insertMovieDirectors = (
  ...
  const insertQueries = directors.map((director) => {
    const matchingDirectorRow = directorRows.find(
      (row) => row.full_name === director
    );
    if (!matchingDirectorRow) {
      throw new Error(`Director not found: ${director}`);
    }
    return `INSERT INTO ${MOVIE_DIRECTORS} (movie_id, director_id) VALUES (${movieId}, ${matchingDirectorRow.id});`;
  });
  return insertQueries.join("\n");
};
...
const insertMovieKeywords = (
  ...
  const insertQueries = keywords.map((keyword) => {
    const matchingKeywordRow = keywordRows.find((row) => row.keyword === keyword);
    if (!matchingKeywordRow) {
      throw new Error(`Keyword not found: ${keyword}`);
    }
    return `INSERT INTO ${MOVIE_KEYWORDS} (movie_id, keyword_id) VALUES (${movieId}, ${matchingKeywordRow.id});`;
  });
  return insertQueries.join("\n");
};
...
const insertMovieProductionCompanies = (
  ...
  const insertQueries = productionCompanies.map((company) => {
    const matchingCompanyRow = productionCompanyRows.find(
      (row) => row.company_name === company
    );
    if (!matchingCompanyRow) {
      throw new Error(`Production company not found: ${company}`);
    }
    return `INSERT INTO ${MOVIE_PRODUCTION_COMPANIES} (movie_id, company_id) VALUES (${movieId}, ${matchingCompanyRow.id});`;
  });
  return insertQueries.join("\n");
};
  ...
  it(
    ...
    async (done) => {
      ...
      for (const imdbId of Object.keys(moviesByImdbId)) {
        const movieId = (await db.selectSingleRow(selectMovieId(imdbId)))
          .id as number;
        const productionCompanies = movies.find(
          (it) => it.imdbId === imdbId
        )!.productionCompanies;
